    	...
    	<logic:iterate name="BPhanLoaiTrailers" id="bean" type="com.form.disability.FPhanLoai">                                       
            <tr class="<%=(i%2==0)?"content1":"content"%>">
              ...
              <td align="center">
	            ...
	            <td width="40px">
                      <img style="border:0px;cursor:pointer;" src="<%=contextPath%>/images/editdraft.png" title="<bean:message key="action.edit" bundle="<%=interfaces%>"/>" onClick="javascript:postAjax('kpi','MainCate',anchor + ':_DETAIL_PHANLOAI:plDtlId:<%=bean.getId()%>')">		            
                      <img style="border:0px;cursor:pointer;" src="<%=contextPath%>/images/delete.png" title="<bean:message key="action.delete" bundle="<%=interfaces%>"/>" onClick="javascript:if(messageDelete())postAjax('kpi','MainCate',anchor + ':_DELETE_PHANLOAI:plDtlId:<%=bean.getId()%>')">
	            </td>
            </tr>
    	</logic:iterate>
    </logic:present>
